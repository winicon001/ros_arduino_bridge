...
// Initialise Motor Control to stop position by default
  ...
  void setMotorSpeed(int i, int spd) {
    ...
    if (i == LEFT) { 
      ...
      else if (reverse == 0) {digitalWrite(LEFT_MOTOR_CTRL, HIGH); analogWrite(LEFT_MOTOR_PWM, 255-spd);} // Right Wheels = Motor 1 on Test Code
    ...
    //   // Left Wheel Reverse if Reverse Bit == 1
      ...
      else if (reverse == 0) {digitalWrite(RIGHT_MOTOR_CTRL, HIGH); analogWrite(RIGHT_MOTOR_PWM, 255-spd);} // Left Wheels = Motor 1 on Test Code
    }
